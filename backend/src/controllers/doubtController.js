...
export async function postStudentDoubt(req,res){
  try{
    ...
    const aiDraftResult = await draftDoubtAnswer(
      problem.title,problem.description,studentCode,studentLanguage,title,content
    ).then(async(aiDraftResult) =>{
    ...
    }).catch(err => console.error("Background AI drafting failed:",err));
